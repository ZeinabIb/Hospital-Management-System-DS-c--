...
void password(){
    ...
    cin>>pass;
        ...
        if(regex_match(pass,regex("^(?=.[A-Za-z])(?=.\\d)[A-Za-z\\d]{8,}$"))){}
        else{
            cout<<"error : password should be at least 8 character long - use capital letters and";
            cout<<"should include numbers"<<endl;
            cout<<"password :";
            cin>>pass;
    }
...
void addPatient(fstream& , Patient *&, string, basic_string<char>, int, string);
...
fstream r("radiologist.txt", ios::in | ios::out | ios::app);
fstream m("medicalHistory.txt", ios::in | ios::out | ios::app);
int main(){
    ...
    addPatient(p,patient,"PatientName","4556",58,"O+");
...
password();
    ...
    addPatient(p,patient,"Angelo Cooper","24566",849,"AB+");
    addPatient(p,patient,"Lesly Greene","894567",456,"O+");
...
void addPatient(fstream& f, Patient *&patient, string fullname, basic_string<char> ins_nb, int id, string blood){
    Patient *temp;
    temp = new Patient;
    if(temp == NULL){
        exit(1);
    ...
    f <<  setw(5) << "fullName : " <<temp->fullName << setw(5) <<"\tInsurance nb : " << temp->ins_nb << setw(5)<<"\tID : "<<temp->id<<setw(5)<<"\tBlood Type : "<<temp->bloodType<<endl;
}
...
void PatientRegistration(){
    ...
    while(true) {
        ...
        if (regex_match(Ins, integer_expr))
            break;
        else { cout << "Invalid input : Not an integer" << endl; }
    }
...
Radiology *searchRadiologist(Radiology *rad, string name) {
    ...
    void BookRadiologyScan(Radiology *rad) {
        ...
        cin >> name;
        int room = rand() % 6 + 1;
        r<<"Appointment with DR."<<name<<"\t\t\t"<<"Type of scan : " << "\tRoom : A" << room << endl;
    ...
    void MedicalHistory(Patient *patient) {
        ...
        //while (cur != NULL) {
          //  if ((cur->fullName).compare(pname) == 0) {
                found = true;
                m << "Patient Name : " << pname << endl;
                cout << "Do The patient Have any Allegrie ?(yes/no) : ";
                cin >> al;
                m << "Do Have Allegrie :" << al << "\t ";
                if (al.compare("yes") == 0) {
                    patient->MH.allergie = true;
                    cout << "Precise Allegie Type(Food Allergy - Pet Allergy - Medication Allergy) :\n";
                    cin >> altype;
                    m << "Type of Allergy : " << altype;

                    cout << "\n-----------Replace Type of Allegy : ";
                    regex regexp("f[a-zA-z]+");
                    cout << "\nReplace the word 'food' with word 'sensitization' \n\n ";
                    cout << regex_replace(altype, regexp, "food");
                } else {
    ...
    void Appointment::SpecificDoctorAppointment() {
        ...
        cout << "Appointment : On " << sday << " in " << smonth << "\tAt: " << doctor->appointment.hour << ":"
             << doctor->appointment.min << am << endl;
    ...
    void PatientBloodType(Patient *patient, string bloodtype) {
        ...
        while (cur != NULL) {
            if ((cur->bloodType).compare(bloodtype) == 0) {
                cout << cur->fullName << "\t\t" << cur->ins_nb << "\t\t\t" << cur->id << "\t\t" << cur->bloodType
                     << endl;
